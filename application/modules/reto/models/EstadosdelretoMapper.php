 ...
 * @author EasyDev:Team
 **/
class Reto_Model_EstadosdelretoMapper extends Model_DataMapperAbstract
{
    ...
    public function  __construct() {
		$this->_objectFilter = new Reto_Model_Estadosdelreto();		
		$this->_primarykey = 'estadosdelreto_id';
   		$this->_nametable = $this->_prefijo.'_estadosdelreto';
    }    
	...
	*/
    public function getById($id){
    	if( !$id > 0)return new Reto_Model_Estadosdelreto();
    	$db = $this->getDb();
	...
	public function UpdateData($data){
		$db = $this->getDb();
		$where = array(0 => "estadosdelreto_id = ".$data["estadosdelreto_id"]);
    ...
    public function DeleteData($id){
    	$db = $this->getDb();
   		$where = array(0 => "estadosdelreto_id = ".$id);
	...
	protected function BuildWhere($select){
		...
		if(strlen($this->_objectFilter->get_Fbuscar())>1){
			$select->where(' a.nombre like "%'.$this->_objectFilter->get_Fbuscar().'%" ');
		}
		if(count($this->_objectFilter->getOrderBy())>0){
			$select->order($this->_objectFilter->getOrderBy());
		}else{

            $select->order("estadosdelreto_id desc");
		}
	...
	public function _populate($data){
		$object = new Reto_Model_Estadosdelreto();
		if($data == null)return $object;
		$label = null;
		if(array_key_exists("estadosdelreto_id", $data))$object->setId($data["estadosdelreto_id"]);
	...
	public function _depopulate($object){
		...
		$_array["estadosdelreto_id"]=$object->getId();
