 ...
 * @author EasyDev:Team
 **/
class Reto_Model_OtrosdesarrollosMapper extends Model_DataMapperAbstract
{
    ...
    public function  __construct() {
		$this->_objectFilter = new Reto_Model_Otrosdesarrollos();		
		$this->_primarykey = 'otrosdesarrollos_id';
   		$this->_nametable = $this->_prefijo.'_otrosdesarrollos';
    }    
	...
	*/
    public function getById($id){
    	if( !$id > 0)return new Reto_Model_Otrosdesarrollos();
    	$db = $this->getDb();
	...
	public function UpdateData($data){
		$db = $this->getDb();
		$where = array(0 => "otrosdesarrollos_id = ".$data["otrosdesarrollos_id"]);
    ...
    public function DeleteData($id){
    	$db = $this->getDb();
   		$where = array(0 => "otrosdesarrollos_id = ".$id);
	...
	protected function BuildWhere($select){
		...
		if(strlen($this->_objectFilter->get_Fbuscar())>1){
			$select->where(' a.desarrollocomplementario like "%'.$this->_objectFilter->get_Fbuscar().'%" OR a.porque like "%'.$this->_objectFilter->get_Fbuscar().'%" ');
		}
		if(count($this->_objectFilter->getOrderBy())>0){
			$select->order($this->_objectFilter->getOrderBy());
		}else{

            $select->order("otrosdesarrollos_id desc");
		}
	...
	public function _populate($data){
		$object = new Reto_Model_Otrosdesarrollos();
		if($data == null)return $object;
		$label = null;
		if(array_key_exists("otrosdesarrollos_id", $data))$object->setId($data["otrosdesarrollos_id"]);
		if(array_key_exists("desarrollocomplementario", $data))$object->setDesarrollocomplementario($data["desarrollocomplementario"]);
		if(array_key_exists("porque", $data))$object->setPorque($data["porque"]);
	...
	public function _depopulate($object){
		...
		$_array["otrosdesarrollos_id"]=$object->getId();
		$_array["desarrollocomplementario"] = $object->getDesarrollocomplementario();
		$_array["porque"] = $object->getPorque();
