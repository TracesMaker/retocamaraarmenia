 ...
 * @author EasyDev:Team
 **/
class Reto_Model_RiesgostecnicosMapper extends Model_DataMapperAbstract
{
    ...
    public function  __construct() {
		$this->_objectFilter = new Reto_Model_Riesgostecnicos();		
		$this->_primarykey = 'riesgostecnicos_id';
   		$this->_nametable = $this->_prefijo.'_riesgostecnicos';
    }    
	...
	*/
    public function getById($id){
    	if( !$id > 0)return new Reto_Model_Riesgostecnicos();
    	$db = $this->getDb();
	...
	public function UpdateData($data){
		$db = $this->getDb();
		$where = array(0 => "riesgostecnicos_id = ".$data["riesgostecnicos_id"]);
    ...
    public function DeleteData($id){
    	$db = $this->getDb();
   		$where = array(0 => "riesgostecnicos_id = ".$id);
	...
	protected function BuildWhere($select){
		...
		if(strlen($this->_objectFilter->get_Fbuscar())>1){
			$select->where(' a.nombredelriesgo like "%'.$this->_objectFilter->get_Fbuscar().'%" OR a.descripcion like "%'.$this->_objectFilter->get_Fbuscar().'%" OR a.probabilidad like "%'.$this->_objectFilter->get_Fbuscar().'%" OR a.impacto like "%'.$this->_objectFilter->get_Fbuscar().'%" OR a.prevencion like "%'.$this->_objectFilter->get_Fbuscar().'%" ');
		}
		if(count($this->_objectFilter->getOrderBy())>0){
			$select->order($this->_objectFilter->getOrderBy());
		}else{

            $select->order("riesgostecnicos_id desc");
		}
	...
	public function _populate($data){
		$object = new Reto_Model_Riesgostecnicos();
		if($data == null)return $object;
		$label = null;
		if(array_key_exists("riesgostecnicos_id", $data))$object->setId($data["riesgostecnicos_id"]);
		if(array_key_exists("nombredelriesgo", $data))$object->setNombredelriesgo($data["nombredelriesgo"]);
		if(array_key_exists("descripcion", $data))$object->setDescripcion($data["descripcion"]);
		if(array_key_exists("probabilidad", $data))$object->setProbabilidad($data["probabilidad"]);
		if(array_key_exists("impacto", $data))$object->setImpacto($data["impacto"]);
		if(array_key_exists("prevencion", $data))$object->setPrevencion($data["prevencion"]);
	...
	public function _depopulate($object){
		...
		$_array["riesgostecnicos_id"]=$object->getId();
		$_array["nombredelriesgo"] = $object->getNombredelriesgo();
		$_array["descripcion"] = $object->getDescripcion();
		$_array["probabilidad"] = $object->getProbabilidad();
		$_array["impacto"] = $object->getImpacto();
		$_array["prevencion"] = $object->getPrevencion();
