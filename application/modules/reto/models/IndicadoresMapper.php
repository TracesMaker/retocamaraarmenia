 ...
 * @author EasyDev:Team
 **/
class Reto_Model_IndicadoresMapper extends Model_DataMapperAbstract
{
    ...
    public function  __construct() {
		$this->_objectFilter = new Reto_Model_Indicadores();		
		$this->_primarykey = 'indicadores_id';
   		$this->_nametable = $this->_prefijo.'_indicadores';
    }    
	...
	*/
    public function getById($id){
    	if( !$id > 0)return new Reto_Model_Indicadores();
    	$db = $this->getDb();
	...
	public function UpdateData($data){
		$db = $this->getDb();
		$where = array(0 => "indicadores_id = ".$data["indicadores_id"]);
    ...
    public function DeleteData($id){
    	$db = $this->getDb();
   		$where = array(0 => "indicadores_id = ".$id);
	...
	protected function BuildWhere($select){
		...
		if(strlen($this->_objectFilter->get_Fbuscar())>1){
			$select->where(' a.indicador like "%'.$this->_objectFilter->get_Fbuscar().'%" OR a.descripcion like "%'.$this->_objectFilter->get_Fbuscar().'%" ');
		}
		if(count($this->_objectFilter->getOrderBy())>0){
			$select->order($this->_objectFilter->getOrderBy());
		}else{

            $select->order("indicadores_id desc");
		}
	...
	public function _populate($data){
		$object = new Reto_Model_Indicadores();
		if($data == null)return $object;
		$label = null;
		if(array_key_exists("indicadores_id", $data))$object->setId($data["indicadores_id"]);
		if(array_key_exists("indicador", $data))$object->setIndicador($data["indicador"]);
	...
	public function _depopulate($object){
		...
		$_array["indicadores_id"]=$object->getId();
		$_array["indicador"] = $object->getIndicador();
