 ...
 * @author EasyDev:Team
 **/
class Reto_Model_ElementostangiblesMapper extends Model_DataMapperAbstract
{
    ...
    public function  __construct() {
		$this->_objectFilter = new Reto_Model_Elementostangibles();		
		$this->_primarykey = 'elementostangibles_id';
   		$this->_nametable = $this->_prefijo.'_elementostangibles';
    }    
	...
	*/
    public function getById($id){
    	if( !$id > 0)return new Reto_Model_Elementostangibles();
    	$db = $this->getDb();
	...
	public function UpdateData($data){
		$db = $this->getDb();
		$where = array(0 => "elementostangibles_id = ".$data["elementostangibles_id"]);
    ...
    public function DeleteData($id){
    	$db = $this->getDb();
   		$where = array(0 => "elementostangibles_id = ".$id);
	...
	protected function BuildWhere($select){
		...
		if(strlen($this->_objectFilter->get_Fbuscar())>1){
			$select->where(' a.elementofisico like "%'.$this->_objectFilter->get_Fbuscar().'%" OR a.descripcion like "%'.$this->_objectFilter->get_Fbuscar().'%" OR a.funcionenlasolucion like "%'.$this->_objectFilter->get_Fbuscar().'%" ');
		}
		if(count($this->_objectFilter->getOrderBy())>0){
			$select->order($this->_objectFilter->getOrderBy());
		}else{

            $select->order("elementostangibles_id desc");
		}
	...
	public function _populate($data){
		$object = new Reto_Model_Elementostangibles();
		if($data == null)return $object;
		$label = null;
		if(array_key_exists("elementostangibles_id", $data))$object->setId($data["elementostangibles_id"]);
		if(array_key_exists("elementofisico", $data))$object->setElementofisico($data["elementofisico"]);
		if(array_key_exists("descripcion", $data))$object->setDescripcion($data["descripcion"]);
		if(array_key_exists("funcionenlasolucion", $data))$object->setFuncionenlasolucion($data["funcionenlasolucion"]);
	...
	public function _depopulate($object){
		...
		$_array["elementostangibles_id"]=$object->getId();
		$_array["elementofisico"] = $object->getElementofisico();
		$_array["descripcion"] = $object->getDescripcion();
		$_array["funcionenlasolucion"] = $object->getFuncionenlasolucion();
