 ...
 * @author EasyDev:Team
 **/
class Reto_Model_GrupoevaluacionMapper extends Model_DataMapperAbstract
{
    ...
    public function  __construct() {
		$this->_objectFilter = new Reto_Model_Grupoevaluacion();		
		$this->_primarykey = 'grupoevaluacion_id';
   		$this->_nametable = $this->_prefijo.'_grupoevaluacion';
    }    
	...
	*/
    public function getById($id){
    	if( !$id > 0)return new Reto_Model_Grupoevaluacion();
    	$db = $this->getDb();
	...
	public function UpdateData($data){
		$db = $this->getDb();
		$where = array(0 => "grupoevaluacion_id = ".$data["grupoevaluacion_id"]);
    ...
    public function DeleteData($id){
    	$db = $this->getDb();
   		$where = array(0 => "grupoevaluacion_id = ".$id);
	...
	protected function BuildWhere($select){
		...
		if(strlen($this->_objectFilter->get_Fbuscar())>1){
			$select->where(' a.nombre like "%'.$this->_objectFilter->get_Fbuscar().'%" OR a.orden like "%'.$this->_objectFilter->get_Fbuscar().'%" ');
		}
		if(count($this->_objectFilter->getOrderBy())>0){
			$select->order($this->_objectFilter->getOrderBy());
		}else{

            $select->order("grupoevaluacion_id desc");
		}
	...
	public function _populate($data){
		$object = new Reto_Model_Grupoevaluacion();
		if($data == null)return $object;
		$label = null;
		if(array_key_exists("grupoevaluacion_id", $data))$object->setId($data["grupoevaluacion_id"]);
		if(array_key_exists("nombre", $data))$object->setNombre($data["nombre"]);
		if(array_key_exists("orden", $data))$object->setOrden($data["orden"]);
		if(array_key_exists("peso", $data))$object->setPeso($data["peso"]);
	...
	public function _depopulate($object){
		...
		$_array["grupoevaluacion_id"]=$object->getId();
		$_array["nombre"] = $object->getNombre();
		$_array["orden"] = $object->getOrden();
		$_array["peso"] = $object->getPeso();
 		return $_array;
	}
