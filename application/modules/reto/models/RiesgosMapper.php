 ...
 * @author EasyDev:Team
 **/
class Reto_Model_RiesgosMapper extends Model_DataMapperAbstract
{
    ...
    public function  __construct() {
		$this->_objectFilter = new Reto_Model_Riesgos();		
		$this->_primarykey = 'riesgos_id';
   		$this->_nametable = $this->_prefijo.'_riesgos';
    }    
	...
	*/
    public function getById($id){
    	if( !$id > 0)return new Reto_Model_Riesgos();
    	$db = $this->getDb();
	...
	public function UpdateData($data){
		$db = $this->getDb();
		$where = array(0 => "riesgos_id = ".$data["riesgos_id"]);
	...
	protected function BuildWhere($select){
		...
		if(strlen($this->_objectFilter->get_Fbuscar())>1){
			$select->where(' a.riesgo like "%'.$this->_objectFilter->get_Fbuscar().'%" OR a.descripcion like "%'.$this->_objectFilter->get_Fbuscar().'%" OR a.impacto like "%'.$this->_objectFilter->get_Fbuscar().'%" OR a.probabilidad like "%'.$this->_objectFilter->get_Fbuscar().'%" ');
		}
		if(count($this->_objectFilter->getOrderBy())>0){
			$select->order($this->_objectFilter->getOrderBy());
		}else{

            $select->order("riesgos_id desc");
		}
	...
	public function _populate($data){
		$object = new Reto_Model_Riesgos();
		if($data == null)return $object;
		$label = null;
		if(array_key_exists("riesgos_id", $data))$object->setId($data["riesgos_id"]);
		if(array_key_exists("riesgo", $data))$object->setRiesgo($data["riesgo"]);
	...
	public function _depopulate($object){
		...
		$_array["riesgos_id"]=$object->getId();
		$_array["riesgo"] = $object->getRiesgo();
