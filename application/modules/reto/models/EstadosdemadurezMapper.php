 ...
 * @author EasyDev:Team
 **/
class Reto_Model_EstadosdemadurezMapper extends Model_DataMapperAbstract
{
    ...
    public function  __construct() {
		$this->_objectFilter = new Reto_Model_Estadosdemadurez();		
		$this->_primarykey = 'estadosdemadurez_id';
   		$this->_nametable = $this->_prefijo.'_estadosdemadurez';
    }    
	...
	*/
    public function getById($id){
    	if( !$id > 0)return new Reto_Model_Estadosdemadurez();
    	$db = $this->getDb();
	...
	public function UpdateData($data){
		$db = $this->getDb();
		$where = array(0 => "estadosdemadurez_id = ".$data["estadosdemadurez_id"]);
    ...
    public function DeleteData($id){
    	$db = $this->getDb();
   		$where = array(0 => "estadosdemadurez_id = ".$id);
	...
	public function _populate($data){
		$object = new Reto_Model_Estadosdemadurez();
		if($data == null)return $object;
		$label = null;
		if(array_key_exists("estadosdemadurez_id", $data))$object->setId($data["estadosdemadurez_id"]);
	...
	public function _depopulate($object){
		...
		$_array["estadosdemadurez_id"]=$object->getId();
