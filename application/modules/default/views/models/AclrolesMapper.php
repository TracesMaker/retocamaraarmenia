 ...
 * @author EasyDev:Team
 **/
class Model_AclrolesMapper extends Model_DataMapperAbstract
{
    ...
    public function  __construct() {
		$this->_objectFilter = new Model_Aclroles();		
		$this->_primarykey = 'aclroles_id';
   		$this->_nametable = $this->_prefijo.'_aclroles';
    }    
	...
	*/
    public function getById($id){
    	if( !$id > 0)return new Model_Aclroles();
    	$db = $this->getDb();
	...
	public function UpdateData($data){
		$db = $this->getDb();
		$where = array(0 => "aclroles_id = ".$data["aclroles_id"]);
    ...
    public function DeleteData($id){
    	$db = $this->getDb();
   		$where = array(0 => "aclroles_id = ".$id);
		$db->delete($this->_nametable, $where);		
	}


	protected function BuildWhere($select){
		if($this->_objectFilter->getRolpadre() > 0) 
			$select->where("a.rolpadre = ?",$this->_objectFilter->getRolpadre());

		if(strlen($this->_objectFilter->get_Fbuscar())>1){
			$select->where(' a.rolnombre like "%'.$this->_objectFilter->get_Fbuscar().'%" ');
		}
		if(count($this->_objectFilter->getOrderBy())>0){
			$select->order($this->_objectFilter->getOrderBy());
		}else{

            $select->order("aclroles_id desc");
		}
	...
	public function _populate($data){
		$object = new Model_Aclroles();
		if($data == null)return $object;
		$label = null;
		if(array_key_exists("aclroles_id", $data))$object->setId($data["aclroles_id"]);
		if(array_key_exists("rolnombre", $data))$object->setRolnombre($data["rolnombre"]);
	if(array_key_exists("rolpadre", $data)){
		$object->setRolpadre($data["rolpadre"]);
		if($this->getPerezoso()){
			$rolpadreDB = new Model_AclrolesMapper();
			$rolpadreDB->setPerezoso(false);
			//$objectRelacion = $rolpadreDB->getById(0);	
			$objectRelacion = $rolpadreDB->getById($data["rolpadre"]);	
			$object->setRolpadreObject($objectRelacion);
		}	
	}
	...
	public function _depopulate($object){
		...
		$_array["aclroles_id"]=$object->getId();
		$_array["rolnombre"] = $object->getRolnombre();
		$_array["rolpadre"] = $object->getRolpadre();
 		return $_array;
	}
