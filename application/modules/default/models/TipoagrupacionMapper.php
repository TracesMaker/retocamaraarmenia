 ...
 * @author EasyDev:Team
 **/
class Model_TipoagrupacionMapper extends Model_DataMapperAbstract
{
    ...
    public function  __construct() {
		$this->_objectFilter = new Model_Tipoagrupacion();		
		$this->_primarykey = 'tipoagrupacion_id';
   		$this->_nametable = $this->_prefijo.'_tipoagrupacion';
    }    
	...
	*/
    public function getById($id){
    	if( !$id > 0)return new Model_Tipoagrupacion();
    	$db = $this->getDb();
	...
	public function UpdateData($data){
		$db = $this->getDb();
		$where = array(0 => "tipoagrupacion_id = ".$data["tipoagrupacion_id"]);
    ...
    public function DeleteData($id){
    	$db = $this->getDb();
   		$where = array(0 => "tipoagrupacion_id = ".$id);
	...
	protected function BuildWhere($select){
		...
		if(strlen($this->_objectFilter->get_Fbuscar())>1){
			$select->where(' a.tanombre like "%'.$this->_objectFilter->get_Fbuscar().'%" ');
		}
		if(count($this->_objectFilter->getOrderBy())>0){
			$select->order($this->_objectFilter->getOrderBy());
		}else{

            $select->order("tipoagrupacion_id desc");
		}
	...
	public function _populate($data){
		$object = new Model_Tipoagrupacion();
		if($data == null)return $object;
		$label = null;
		if(array_key_exists("tipoagrupacion_id", $data))$object->setId($data["tipoagrupacion_id"]);
		if(array_key_exists("tanombre", $data))$object->setTanombre($data["tanombre"]);
	...
	public function _depopulate($object){
		...
		$_array["tipoagrupacion_id"]=$object->getId();
		$_array["tanombre"] = $object->getTanombre();
 		return $_array;
	}
