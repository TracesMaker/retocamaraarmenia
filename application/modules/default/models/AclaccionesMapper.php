 ...
 * @author EasyDev:Team
 **/
class Model_AclaccionesMapper extends Model_DataMapperAbstract
{
    ...
    public function  __construct() {
		$this->_objectFilter = new Model_Aclacciones();		
		$this->_primarykey = 'aclacciones_id';
   		$this->_nametable = $this->_prefijo.'_aclacciones';
    }    
	...
	*/
    public function getById($id){
    	if( !$id > 0)return new Model_Aclacciones();
    	$db = $this->getDb();
	...
	public function UpdateData($data){
		$db = $this->getDb();
		$where = array(0 => "aclacciones_id = ".$data["aclacciones_id"]);
    ...
    public function DeleteData($id){
    	$db = $this->getDb();
   		$where = array(0 => "aclacciones_id = ".$id);
	...
	protected function BuildWhere($select){
		...
		if(strlen($this->_objectFilter->get_Fbuscar())>1){
			$select->where(' a.accrecurso like "%'.$this->_objectFilter->get_Fbuscar().'%" OR a.accaccion like "%'.$this->_objectFilter->get_Fbuscar().'%" ');
		}
		if(count($this->_objectFilter->getOrderBy())>0){
			$select->order($this->_objectFilter->getOrderBy());
		}else{

            $select->order("aclacciones_id desc");
		}
	...
	public function _populate($data){
		$object = new Model_Aclacciones();
		if($data == null)return $object;
		$label = null;
		if(array_key_exists("aclacciones_id", $data))$object->setId($data["aclacciones_id"]);
		if(array_key_exists("accrecurso", $data))$object->setAccrecurso($data["accrecurso"]);
		if(array_key_exists("accaccion", $data))$object->setAccaccion($data["accaccion"]);
	...
	public function _depopulate($object){
		...
		$_array["aclacciones_id"]=$object->getId();
		$_array["accrecurso"] = $object->getAccrecurso();
		$_array["accaccion"] = $object->getAccaccion();
 		return $_array;
	}
